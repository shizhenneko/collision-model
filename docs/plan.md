...
### 2.5 多时间尺度预测机制 (Multi-Time Scale Prediction)
为解决**ANN强制停止后VOA动作续接**问题，引入多时间尺度预测：

*   **问题场景**: 当ANN预测高碰撞风险时，系统执行紧急制动($v=0, \omega=0$)。此时VOA需要预测**危险解除后的后续动作**，而非原地不动。

*   **解决方案**: 
    *   **短期预测** ($t+0.1s$): 应对即时环境变化
    *   **中期预测** ($t+0.5s$): 覆盖ANN制动周期
    *   **长期预测** ($t+1.0s$): 规划危险解除后的恢复动作

*   **训练标签生成**: 基于专家遥控指令序列，提取未来多个时间点的动作对：
    ```
    输入: 当前状态 $S_t$
    输出: $[v_{t+0.1}, \omega_{t+0.1}, v_{t+0.5}, \omega_{t+0.5}, v_{t+1.0}, \omega_{t+1.0}]$
    ```

*   **推理流程**:
    1. 正常状态: 使用短期预测 $[v_{t+0.1}, \omega_{t+0.1}]$ 作为当前动作
    2. ANN触发: 执行紧急制动，同时启动计时器
    3. 危险解除: 根据制动持续时间选择合适时间尺度的预测动作
    4. 平滑过渡: 使用线性插值确保动作连续性

### 2.6 任务头与训练目标 (Task Heads & Labels)
...
| **VOA Multi-Scale** (多尺度策略) | MLP | $[v_1, \omega_1, v_2, \omega_2, v_3, \omega_3]$ | **多时间尺度模仿学习** | **人工遥控指令序列**。预测未来多个时间尺度的动作序列，支持ANN强制停止后的动作续接。 |
...
3.  **Training**:
    ...
    *   **Step 2**: 训练 **VOA 多时间尺度策略网络**，基于多时间尺度专家动作序列学习预测能力。
    *   **Step 3**: 训练 **VOA 策略网络**，基于单时间尺度MSE Loss 学习专家的驾驶动作。
...
4.  **Multi-Time Scale Action Recovery**: 当ANN触发紧急制动后，系统根据制动持续时间自动选择VOA多时间尺度预测中的合适动作进行恢复，避免机器人原地不动。
